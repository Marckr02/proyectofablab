...
const GallerySection: React.FC = () => {
    ...
    return (
        <section className="space-y-12">
            <div>
                <h1 className="text-3xl sm:text-4xl font-extrabold text-white text-center mb-4 flex items-center justify-center gap-4">
                    <GalleryIcon /> Galería de Proyectos
                </h1>
                <p className="text-lg text-slate-400 text-center max-w-3xl mx-auto">
                    Explora una selección de modelos y creaciones realizadas con la Halot Mage Pro en el FabLab.
                </p>
            </div>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
                {GALLERY_ITEMS.map((item, index) => (
                    <GalleryCard key={item.id} item={item} onSelect={() => handleSelect(index)} />
                ))}
            </div>
